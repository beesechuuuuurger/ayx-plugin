...
# Set up logging
logging.basicConfig(filename='plugin_log.log', level=logging.DEBUG,
...
class GPT(PluginV2):
    def __init_plugin__(self, provider):
        self.name = "GPT"
        ...
        # Add instance variables for the UI controls
        self.api_key_control = None
        self.input_column_control = None
        self.optional_input_control = None
        self.classification_attributes_control = None
        self.prompt_control = None
        self.output_format_control = None
        self.alteryx_engine.register_call_back(
            "sendDataToPython", self.on_receive_data_from_ui)
    ...
    def initialize_ui_elements(self):
        ...
        self.api_key_control = self.alteryx_engine.create_control(
            self.n_tool_id, 0, "api_key", Sdk.ControlType.text_box)
        self.api_key_control.tool_tip = "Enter your OpenAI API Key"

        self.input_column_control = self.alteryx_engine.create_control(
            self.n_tool_id, 1, "input_column", Sdk.ControlType.text_box)
        self.input_column_control.tool_tip = "Enter the name of the input column to classify"

        self.optional_input_control = self.alteryx_engine.create_control(
            self.n_tool_id, 2, "optional_input", Sdk.ControlType.text_box)
        self.optional_input_control.tool_tip = "Enter any optional input to aid in classification"

        self.classification_attributes_control = self.alteryx_engine.create_control(
            self.n_tool_id, 3, "classification_attributes", Sdk.ControlType.text_box)
        self.classification_attributes_control.tool_tip = "Enter a comma-separated list of attributes to classify"

        self.prompt_control = self.alteryx_engine.create_control(
            self.n_tool_id, 4, "prompt", Sdk.ControlType.text_box)
        self.prompt_control.tool_tip = "Enter the format for the prompt to be used for classification"

        self.output_format_control = self.alteryx_engine.create_control(
            self.n_tool_id, 5, "output_format", Sdk.ControlType.text_box)
        self.output_format_control.tool_tip = "Enter the output format for the classification results"

        # Add the UI elements to the plugin
        self.alteryx_engine.init_tool(self.n_tool_id, [
            self.api_key_control,
            self.input_column_control,
            self.optional_input_control,
            self.classification_attributes_control,
            self.prompt_control,
            self.output_format_control])
    ...
    def on_receive_data_from_ui(self, payload):
        ...
        apiKey = self.api_key_control.value
        inputColumn = self.input_column_control.value
        optionalInput = self.optional_input_control.value
        classificationAttributes = self.classification_attributes_control.value
        prompt = self.prompt_control.value
        outputFormat = self.output_format_control.value
        ...
        full_prompt = f"{optionalInput}\n{prompt}"

        # Make the API call to ChatGPT using Langchain
        response = client.generate_text(
            prompt=full_prompt, model="gpt-3.5-turbo", max_tokens=50)
    ...
    def create_template(self, prompt: str, input_values: list, categories: list = None, classify_or_attributes: str = "Classify"):
        ...
        if self.classify_or_attributes == "Classify":
            ...
            raise ValueError(
                "Invalid value for classify_or_attributes. It must be 'Classify' or 'Attributes'.")
    ...
    def build_optional_prompt(self, input_value, optional_input_value):
        ...
        prompt = f"{self.prompt} {input_value} {optional_input_value}"
        return prompt
